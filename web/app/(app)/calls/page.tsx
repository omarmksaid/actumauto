  ...
  return (
    <div>
      ...
      <p className="page-sub">
        Every call into the service line — play back the recording, read the transcript.
      </p>
      ...
      <div className="card">
        {loading ? <div className="card-pad muted">Loading…</div> : (
          <table>
            <thead><tr><th>Caller</th><th>Outcome</th><th>Duration</th><th>Cost</th><th>When</th></tr></thead>
            <tbody>
              {calls.map((call) => (
                <tr key={call.id} className="rowlink" onClick={() => router.push(`/calls/${call.id}`)}>
                  <td>
                    {call.customers?.full_name ?? <span className="muted">Not identified</span>}
